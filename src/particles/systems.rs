...
pub fn emit_particles(
    ...
    mut spawners: Query<(Entity, &Children, &ParticleSpawner, &mut ParticleSpawnerTimer)>,
        ...
        if timer.lifetime.finished() {
            ...
            for _i in 0..spawner.amount_per_burst {
                for child in children.iter() {
                    if let Ok((mut particle, mut visibility, mut style)) =
...
fn spawn_particle_spawner(
    ...
    let spawner = ron::from_str::<ParticleSpawner>(ron_str).expect("Failed to load basic_spawner.ron");
    ...
    commands
        ...
        .insert(ParticleSpawnerTimer {
            spray_timer: Timer::from_seconds(
                spawner.rate,
                TimerMode::Repeating,
            ),
            ...
            finishing_time: Timer::from_seconds(
                spawner.particle_lifetime,
                TimerMode::Once,
            )
        })
        .insert(spawner)
        .with_children(|parent| {
            ...
            {
                parent.spawn(
                    build_particle(&spawner, &window_query)
                );
            }
        });
}
...
pub fn update_particle_color(mut particles: Query<(&Particle, &ParticleColor, &mut BackgroundColor)>) {
